export const LOCALE = {
    'en': {
        'build_mode': "BUILD MODE:",
        'station_btn': "Station (Path)",
        'mine_btn': "Mine (Points)",
        'title': "Mars Miners: Mobile Edition",
        'setup_title': "Mars Expedition Setup",
        'assign_roles': "Assign Roles for Players:",
        'player': "Player", 'human': "Human", 'easy_ai': "Easy AI", 'normal_ai': "Normal AI", 'hard_ai': "Hard AI", 'none': "None",
        'easy_ai': "Easy AI",
        'normal_ai': "Normal AI",
        'hard_ai': "Hard AI",
        'player_1': "Player 1", 'player_2': "Player 2", 'player_3': "Player 3", 'player_4': "Player 4",
        'map_size': "Map Size:", 'weapon_req': "Weapon Length:",
        'stations': "Stations", 'ai_wait': "AI Turn Wait Time:",
        'asap': "ASAP (0ms)", 'fast': "Fast (500ms)", 'med': "Medium (1000ms)", 'slow': "Slow (2000ms)",
        'allow_skip': "Allow manual Turn Skip", 'start_btn': "Start Mission",
        'singleplayer': "Singleplayer", 'multiplayer': "Multiplayer",
        'session_id': "Session ID", 'user_id': "Unique User ID",
        'share_btn': "Share", 'copy_btn': "Copy", 'join_session': "Join", 'create_session': "Create",
        'resume_btn': "Resume Mission",
        'new_game_btn': "New Game",
        'load_game_btn': "Load Mission",
        'join_game_btn': "Join Game",
        'training_btn': "Training",
        'training_rules_title': "How to play",
        'training_rules_text': "Build stations to expand territory.\nBuild mines to gather points.\nConnect 4 stations in a line to charge a laser.\nTap on your station, then on an enemy to shoot.",
        'start_training_btn': "Start Training",
        'expedition_log': "--- EXPEDITION LOG ---", 'initializing': "Initializing...",
        'game_over': "Mission Complete",
        'save_confirm_title': "Save Battle Log",
        'save_confirm_msg': "Do you want to save the expedition record to a file?",
        'skip_turn': "Skip Turn", 'active': "ACTIVE", 'lost': "BLOCKED",
        'mines': "Mines", 'storage': "--- STORAGE ---", 'save': "Save Log",
        'load': "Load Mission", 'new_game': "New Game", 'restart_game': "Restart Mission",
        'controls': "Tap: Place selected\nShift needed? No.",
        'attack_help': "Attack Enemy:\nTap enemy station",
        'requires': "Requires {n}+ Stations",
        'failed': "MISSION FAILED\nALL ELIMINATED",
        'winner': "WINNER: {name} ({m} Mines)",
        'draw': "DRAW: {names} ({m} Mines)",
        'turn': "TURN: {name}", 'ready': "READY ({n})", 'charging': "CHARGING ({n}/{req})",
        'ai_thinking': "AI thinking...",
        'abandon_msg': "Are you sure you want to abandon the current mission?",
        'abandon_title': "Abandon Mission", 'exit_msg': "Exit game?", 'exit_title': "Exit",
        'lang_label': "Language / Язык:",
        'highlight_weapon': "Highlight weapons",
    },
    'ru': {
        'build_mode': "РЕЖИМ СТРОЙКИ:",
        'station_btn': "Станция (Путь)",
        'mine_btn': "Шахта (Очки)",
        'title': "Марсианские Шахтеры",
        'setup_title': "Настройка экспедиции",
        'assign_roles': "Назначьте роли:",
        'player': "Игрок", 'human': "Человек", 'easy_ai': "Легкий ИИ", 'normal_ai': "Обычный ИИ", 'hard_ai': "Сильный ИИ", 'none': "Нет",
        'easy_ai': "Легкий ИИ",
        'normal_ai': "Обычный ИИ",
        'hard_ai': "Сильный ИИ",
        'player_1': "Игрок 1", 'player_2': "Игрок 2", 'player_3': "Игрок 3", 'player_4': "Игрок 4",
        'map_size': "Размер карты:", 'weapon_req': "Длина орудия:",
        'stations': "Станций", 'ai_wait': "Задержка ИИ:",
        'asap': "0мс", 'fast': "500мс", 'med': "1000мс", 'slow': "2000мс",
        'allow_skip': "Пропуск хода", 'start_btn': "Начать",
        'singleplayer': "Одиночная", 'multiplayer': "Сетевая",
        'session_id': "ID Сессии", 'user_id': "ID Пользователя",
        'share_btn': "Поделиться", 'copy_btn': "Копировать", 'join_session': "Зайти", 'create_session': "Создать",
        'resume_btn': "Продолжить",
        'new_game_btn': "Новая игра",
        'load_game_btn': "Загрузить миссию",
        'join_game_btn': "Присоединиться",
        'training_btn': "Тренировка",
        'training_rules_title': "Как играть",
        'training_rules_text': "Стройте станции для расширения базы,\nшахты — для победных очков.\nПостройте 4 станции в ряд, чтобы зарядить лазер.\nНажмите на свою станцию, затем на вражескую.",
        'start_training_btn': "Начать",
        'expedition_log': "--- ЖУРНАЛ ---", 'initializing': "Загрузка...",
        'game_over': "Миссия выполнена",
        'save_confirm_title': "Сохранить журнал",
        'save_confirm_msg': "Вы хотите сохранить запись экспедиции в файл?",
        'skip_turn': "Пропустить ход", 'active': "АКТИВЕН", 'lost': "БЛОК",
        'mines': "Шахт", 'storage': "--- ХРАНИЛИЩЕ ---", 'save': "Сохр. лог",
        'load': "Загрузить", 'new_game': "Новая игра", 'restart_game': "Заново",
        'controls': "Тап: Строить выбор",
        'attack_help': "Атака:\nТап по врагу",
        'requires': "Нужно {n}+ станций",
        'failed': "МИССИЯ ПРОВАЛЕНА",
        'winner': "ПОБЕДА: {name} ({m} Шахт)",
        'draw': "НИЧЬЯ: {names} ({m} Шахт)",
        'turn': "ХОД: {name}", 'ready': "ГОТОВО ({n})", 'charging': "ЗАРЯДКА ({n}/{req})",
        'ai_thinking': "ИИ думает...",
        'abandon_msg': "Прервать миссию?", 'abandon_title': "Прервать",
        'exit_msg': "Выйти?", 'exit_title': "Выход",
        'lang_label': "Язык:",
        'highlight_weapon': "Подсветка орудий",
    }
};

export function t(key: string, lang: 'en' | 'ru' = 'en', params?: Record<string, any>): string {
    let str = (LOCALE[lang] as any)[key] || (LOCALE['en'] as any)[key] || key;
    if (params) {
        for (const k in params) {
            str = str.replace(`{${k}}`, params[k]);
        }
    }
    return str;
}
